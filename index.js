const record = [
  ...
  function superbowlWin(array){
    //use .find() array method to find any years with W in it and assign it to new variable winYear
    let winYear = array.find(element => element.result === 'W' );
    //if state checks to see if winYear exists, and if it does return the year of the win
    if(winYear){
        return winYear.year;
    }else{ //otherwise, if it doesnt exist, it will return undefined
        return undefined;
    };  
}; 
 superbowlWin(record)
 ...
 record.find(function superbowlWin(record) {
   if (record.result === "W"){
     return record.year
   }
 })
